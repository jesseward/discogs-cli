...
def label(label_id):
    ...
    try:
        ...
        click.secho(
            "Unable to fetch label id: {label_id} ({e})".format(label_id=label_id, e=e),
            fg="red",
        )
...
def release(release_id):
    ...
    try:
        ...
        click.secho(
            "Unable to fetch release id: {release_id} ({e})".format(
                release_id=release_id, e=e
            ),
            fg="red",
        )
...
def artist(artist_id):
    ...
    try:
        ...
        click.secho(
            "Unable to fetch artist id: {artist_id} ({e})".format(
                artist_id=artist_id, e=e
            ),
            fg="red",
        )
...
def master(master_id):
    ...
    try:
        ...
        click.secho(
            "Unable to fetch master id: {master_id} ({e})".format(
                master_id=master_id, e=e
            ),
            fg="red",
        )
...
def search(query, lookup):
    ...
    if not token:
        click.secho(
            "Unable to read your user_token. try export TOKEN=your_discogs_token_here",
            fg="red",
        )
    else:
        ...
        try:
            ...
            click.secho(
                "Unable to perform a {t} search for {q} ({e})".format(
                    t=lookup, q=query, e=e
                ),
                fg="red",
            )
